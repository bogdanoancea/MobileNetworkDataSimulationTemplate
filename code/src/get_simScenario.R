...
get_simScenario <- function(fileGridName = NULL, 
  ...
  if(!is.null(fileEventsInfoName)){
    ...
    setnames(allEvents.dt , c('time', 'antennaID', 'eventCode', 'device', 'x', 'y', 'tile'))
    allEvents.dt <- allEvents.dt[, antennaID := str_pad(antennaID, max(nchar(antennaID)), pad="0")]
  ...
  if(!is.null(fileSignalName)){
    ...
    RSS_Sim.dt <- fread(fileSignalName, sep = ",", header = TRUE, stringsAsFactors = FALSE)
