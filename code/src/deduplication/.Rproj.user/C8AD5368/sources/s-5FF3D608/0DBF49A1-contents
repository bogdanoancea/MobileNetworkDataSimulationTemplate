#' @title Read a file with the posterior location probabilities.
#'
#' @description Read a .csv file with the posterior location probabilities. Each row of the file corresponds to a tile
...
readPostLocProb <-function(path, prefixName, deviceID) {
  ...
  setnames(postLoc, paste0('V', c(1:3, ncol(postLoc))), c('device', 'time', 'tile', 'probL'))
