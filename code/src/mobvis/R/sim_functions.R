...
#' Get simulator data
#'
#' @param sim list containing the following elements: \code{simConfig_dir},
...
sim_get_region <- function(sim) {
    x <- st_as_sfc(readLines(file.path(sim$simConfig_dir, "map.wkt")))
...
sim_get_raster <- function(sim) {
    grd = read.csv(file.path(sim$resources_dir, "grid.csv"))
...
sim_get_cellplan <- function(sim) {
    suppressMessages({
        cellplanxml <- xml2::as_list(xml2::read_xml(file.path(sim$simConfig_dir, "antennas.xml")))
...
sim_get_trajectory_data <- function(sim, device = NULL) {
    f <- file.path(sim$networkEvents_dir, paste0("AntennaInfo_MNO_", sim$mno, ".csv"))
