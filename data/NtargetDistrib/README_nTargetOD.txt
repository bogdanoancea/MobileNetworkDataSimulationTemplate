...
3. <specs_time type=...> 
It describes the timestamp variable and its parent is <nTargetOD>.
...
individuals moving from one region to another and its parent is <nTargetOD>.
It contains two mandatory child elements and one optional element:
a) <nTargetODColName> : mandatory, it gives the column name of the random values generated from the distribution of the number of individuals detected by the network;
b) <nTargetOD_value_type> : it specifies the type of the data on the column and it could take one of the following values: decimal, integer;
c) <description> : an optional tag that can provide a textual description;


6. <specs_iter>
It gives the index of the above mentioned random values and its parent is <nTargetOD>.
It contains two mandatory child elements and one optional element:
...
7. <specs_distr type = ...>
It gives the name of the distribution used to generate the random value and its parent is <nTargetOD>.
